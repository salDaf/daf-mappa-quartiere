...
def compute_vitality_cpa2011(quartiereData):
    ...
    out[newColumnsNames[0]] = ([1, 2, 3, 4] * quartiereData[buildingColumns]).sum(
        axis=1) / quartiereData[buildingColumns].sum(axis=1)
...
def wrangle_istat_cpa2011(quartiereData, selectedCity):
    ...
    for idx, continente in enumerate(list_columns_continenti):
        new_index(quartiereData, [list_attr_continenti[idx]], [
    ...
    donne_anziane = quartiereData[['P27', 'P28', 'P29']].sum(
        axis=1) - (quartiereData[['P43', 'P44', 'P45']].sum(axis=1))
    donne_giovani = quartiereData[['P14', 'P15', 'P16']].sum(
        axis=1) - quartiereData[['P30', 'P31', 'P32']].sum(axis=1)
    # Quindi, il nuovo indice
    quartiereData['indice_vecchiaia_donna'] = donne_anziane / donne_giovani

    # 4. Creo la nuova colonna
    new_index(quartiereData, ['P22', 'P23', 'P24', 'P25', 'P26'], [
              'P17', 'P18', 'P19', 'P20', 'P21'], 'indice_pop_attiva')
    # Creo la nuova colonna
    new_index(quartiereData, ['P38', 'P39', 'P40', 'P41', 'P42'], [
              'P33', 'P34', 'P35', 'P36', 'P37'], 'indice_pop_attiva_uomo')
    # Calcolo numeratore e denominatore
    numeratore_donne = (quartiereData[['P22', 'P23', 'P24', 'P25', 'P26']].sum(
        axis=1)) - (quartiereData[['P38', 'P39', 'P40', 'P41', 'P42']].sum(axis=1))
    denominatore_donne = (quartiereData[['P17', 'P18', 'P19', 'P20', 'P21']].sum(
        axis=1)) - (quartiereData[['P33', 'P34', 'P35', 'P36', 'P37']].sum(axis=1))
    # Creazione colonna
    quartiereData['indice_pop_attiva_donna'] = numeratore_donne / \
        denominatore_donne
    ...
    new_index(quartiereData, ['P137'], ['P1'],
              'indice_pop_non_pend_esterna_quartiere')

    # 7. Creo nuova colonna
    giornata_dentro_quartiere = quartiereData['P1'] - \
        (quartiereData[['P137', 'P138']].sum(axis=1))
    quartiereData['indice_pop_non_pend_interna_quartiere'] = giornata_dentro_quartiere / \
        quartiereData['P1']
