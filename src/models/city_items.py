...
class ServiceType(Enum):
    ...
    def __init__(self, _, areaOfService,
        ...
        assert isinstance(
            demandAgesInput, list), 'list expected for demand ages'
        assert all([isinstance(g, AgeGroup)
                    for g in demandAgesInput]), 'AgeGroups expected'
    ...
    def aggregate_units(self, unitValues, axis=1):
        ...
        return np.apply_along_axis(
            lambda x: np.linalg.norm(
                x, ord=self.aggrNorm.value), axis, unitValues)
